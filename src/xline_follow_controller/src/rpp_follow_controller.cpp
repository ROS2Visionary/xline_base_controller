#include "follow_controller/rpp_follow_controller.hpp"
#include <algorithm>
#include <cmath>
#include <memory>
#include <string>
...
bool RPPController::setPlanForBasePath(const std::shared_ptr<daosnrs::geometry::BasePath>& path_object)
{
  ...
  {
    RCLCPP_ERROR(get_logger(), "收到空的路径对象，无法设置计划");
    return false;
  }

  updateParameters("/config/rpp_curve.yaml");
  is_circle_path = false;
  back_follow_ = false;  // 重置后退模式
  ...
  try
  {
    ...
    // 获取路径消息
    auto path_msg = path_object->toPathMsg();

    if (!path_msg || path_msg->empty())
    {
      RCLCPP_ERROR(get_logger(), "路径对象转换为消息失败或为空");
      return false;
    }

    // 检查路径点有效性
    bool has_invalid_points = false;
    for (size_t i = 0; i < path_msg->size(); ++i)
    {
      if (!std::isfinite(path_msg->at(i).pose.position.x) || !std::isfinite(path_msg->at(i).pose.position.y))
      {
        RCLCPP_ERROR(get_logger(), "路径包含无效点 #%zu: (%f, %f)", i, path_msg->at(i).pose.position.x,
                     path_msg->at(i).pose.position.y);
    ...
    {
      RCLCPP_WARN(get_logger(), "路径包含无效点，尝试清理无效点");
      // 可以在这里实现无效点清理逻辑
    }

    RCLCPP_INFO(get_logger(), "设置新路径 - 类型: %s",
                (std::to_string(static_cast<int>(path_object->type())) + ", 长度: " + std::to_string(path_length_) +
                 "m, 点数: " + std::to_string(path_msg->size()))
                    .c_str());

    // 对于自由规划路径类型，可以进行特殊处理
    if (path_object->type() == daosnrs::geometry::PathType::FreePlan)
    {
      RCLCPP_INFO(get_logger(), "检测到自由规划路径类型");
      RCLCPP_INFO(get_logger(), "处理自由规划路径...");

      // 将路径消息转换为nav_msgs::msg::Path格式
      nav_msgs::msg::Path new_path;
      new_path.header.frame_id = "world";
      new_path.header.stamp = this->now();
      new_path.poses.reserve(path_msg->size());

      // 复制路径点，过滤掉无效点
      for (const auto& pose_stamped : *path_msg)
      {
        // 验证点的有效性
        if (std::isfinite(pose_stamped.pose.position.x) && std::isfinite(pose_stamped.pose.position.y) &&
      ...
      {
        RCLCPP_ERROR(get_logger(), "过滤无效点后路径为空，无法继续");
        return false;
      }

      // 对于自由规划路径，在路径末尾添加额外的点以延伸路径
      if (new_path.poses.size() >= 2)
      {
        const auto& last_pose = new_path.poses.back();
        const auto& prev_last_pose = new_path.poses[new_path.poses.size() - 2];

        // 计算最后两点的方向向量
        double delta_x = last_pose.pose.position.x - prev_last_pose.pose.position.x;
        double delta_y = last_pose.pose.position.y - prev_last_pose.pose.position.y;
        ...
        {
          RCLCPP_WARN(get_logger(), "路径末端点太接近，使用默认方向(1,0)");
          delta_x = 1.0;
          delta_y = 0.0;
        }
        ...
        // 在路径末尾添加额外的点，沿着该方向延伸
        // 高精度模式下添加更多点，间隔更小
        const int extra_points = 10;        // 增加点数
        const double point_spacing = 0.01;  // 1cm间隔，更密集的点以提高精度

        for (int i = 0; i < extra_points; i++)
        {
          geometry_msgs::msg::PoseStamped extended_pose = last_pose;
          extended_pose.pose.position.x += delta_x * (i + 1) * point_spacing;
          extended_pose.pose.position.y += delta_y * (i + 1) * point_spacing;

          // 确保扩展点的朝向与路径方向一致
          tf2::Quaternion q;
          q.setRPY(0, 0, atan2(delta_y, delta_x));
          extended_pose.pose.orientation = tf2::toMsg(q);

          new_path.poses.push_back(extended_pose);
        }

        RCLCPP_INFO(get_logger(), "自由规划路径已延伸 - 额外点数: %s",
                    (std::to_string(extra_points) + ", 总点数: " + std::to_string(new_path.poses.size())).c_str());
      }
      else
      {
        RCLCPP_WARN(get_logger(), "路径点数不足，无法延伸路径");
      }

      // 验证路径点之间的距离
      bool has_large_gaps = false;
      const double max_gap = 0.5;  // 最大允许的两点之间距离（米）

      for (size_t i = 1; i < new_path.poses.size(); i++)
      {
        double dist = std::hypot(new_path.poses[i].pose.position.x - new_path.poses[i - 1].pose.position.x,
                                 new_path.poses[i].pose.position.y - new_path.poses[i - 1].pose.position.y);

        if (dist > max_gap)
        {
          RCLCPP_WARN(get_logger(), "检测到路径点间距过大: 点%zu和点%zu之间距离为%.4f米", i - 1, i, dist);
          has_large_gaps = true;
        }
      }
      ...
      if (has_large_gaps)
      {
        RCLCPP_WARN(get_logger(), "路径存在较大间隙，可能导致控制效果不佳");
        // 这里可以实现路径插值逻辑来填补大间隙
      }
      ...
      {
        // 使用路径起点和第二个点计算切线方向
        const auto& start_point = new_path.poses[0];
        const auto& second_point = new_path.poses[1];

        double delta_x = second_point.pose.position.x - start_point.pose.position.x;
        double delta_y = second_point.pose.position.y - start_point.pose.position.y;

        // 计算切线方向角度
        double tangent_yaw = atan2(delta_y, delta_x);

        // 将角度加π并规范化到[-π, π]范围
        target_yaw_ = tangent_yaw + M_PI;
        target_yaw_ = atan2(sin(target_yaw_), cos(target_yaw_));  // 规范化到[-π, π]

        RCLCPP_INFO(get_logger(), "自由规划路径起点切线方向: %.3f rad (%.1f°), target_yaw_: %.3f rad (%.1f°)",
                    tangent_yaw, tangent_yaw * 180.0 / M_PI, target_yaw_, target_yaw_ * 180.0 / M_PI);
      }
      else
      {
        RCLCPP_WARN(get_logger(), "自由规划路径点数不足，无法计算起点切线方向，使用默认值0");
        target_yaw_ = 0.0;
      }

      // 使用原有的setPlan方法处理转换后的路径
      return setPlan(new_path);
    }
    else
    {
      // 对于其他类型的路径，直接转换为nav_msgs::msg::Path并处理
      nav_msgs::msg::Path new_path;
      new_path.header.frame_id = "world";
      new_path.header.stamp = this->now();
      new_path.poses.reserve(path_msg->size());
      ...
      RCLCPP_INFO(get_logger(), "标准路径处理 - 点数: %s", std::to_string(new_path.poses.size()).c_str());
      ...
      if (new_path.poses.size() >= 2)
      {
        ...
        RCLCPP_INFO(get_logger(), "路径起点切线方向: %.3f rad (%.1f°), target_yaw_: %.3f rad (%.1f°)", tangent_yaw,
                    tangent_yaw * 180.0 / M_PI, target_yaw_, target_yaw_ * 180.0 / M_PI);
      }
      else
      {
        RCLCPP_WARN(get_logger(), "路径点数不足，无法计算起点切线方向，使用默认值0");
        target_yaw_ = 0.0;
      }
  ...
  {
    RCLCPP_ERROR(get_logger(), "设置路径时发生异常: %s", e.what());
    return false;
  }
  catch (...)
  {
    RCLCPP_ERROR(get_logger(), "设置路径时发生未知异常");
    return false;
  }
}
#endif
