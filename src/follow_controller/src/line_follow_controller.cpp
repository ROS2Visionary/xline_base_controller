// =========================== 包含头文件 ===========================
#include "follow_controller/line_follow_controller.hpp"
...
void LineFollowController::initializeDefaultParameters()
{
  ...
  current_roll_angle_ = 0.0;
  filtered_roll_angle_ = 0.0;

  data_log_base_path_ = "/home/daosn_robotics/zyq_ws/terrain_data/";
  enable_detailed_logging_ = true;
  enable_imu_terrain_logging_ = true;
  logging_frequency_ = 20.0;
...
void LineFollowController::updateParameters()
{
  ...
  try
  {
    ...
    virtual_tracking_config_.correction_gain =
        parser.getParameter<double>("sensors.position.virtual_tracking.correction_gain");
    virtual_tracking_config_.max_correction =
        parser.getParameter<double>("sensors.position.virtual_tracking.max_correction");
    virtual_tracking_config_.position_trust_threshold =
        parser.getParameter<double>("sensors.position.virtual_tracking.position_trust_threshold");
    virtual_tracking_config_.max_velocity_trust =
        parser.getParameter<double>("sensors.position.virtual_tracking.kinematics.max_velocity_trust");
    virtual_tracking_config_.angular_velocity_factor =
        parser.getParameter<double>("sensors.position.virtual_tracking.kinematics.angular_velocity_factor");
    virtual_tracking_config_.smooth_gain =
        parser.getParameter<double>("sensors.position.virtual_tracking.terrain_adaptive.smooth_gain");
    virtual_tracking_config_.rough_gain =
        parser.getParameter<double>("sensors.position.virtual_tracking.terrain_adaptive.rough_gain");
    virtual_tracking_config_.sudden_change_gain =
        parser.getParameter<double>("sensors.position.virtual_tracking.terrain_adaptive.sudden_change_gain");
    virtual_tracking_config_.debug_logging =
        parser.getParameter<bool>("sensors.position.virtual_tracking.debug.enable_logging");
    virtual_tracking_config_.log_frequency =
        parser.getParameter<int>("sensors.position.virtual_tracking.debug.log_frequency");

    virtual_tracker_.setParameters(virtual_tracking_config_.correction_gain, virtual_tracking_config_.max_correction,
                                   virtual_tracking_config_.position_trust_threshold,
                                   virtual_tracking_config_.max_velocity_trust,
                                   virtual_tracking_config_.angular_velocity_factor);

    virtual_tracker_.setTerrainGains(virtual_tracking_config_.smooth_gain, virtual_tracking_config_.rough_gain,
                                     virtual_tracking_config_.sudden_change_gain);

    virtual_tracker_.setDebugLogging(virtual_tracking_config_.debug_logging, virtual_tracking_config_.log_frequency);
...
void LineFollowController::initializeFilters()
{
  ...
  angular_vel_hampel_filter_.reset(angular_following_hampel_window_, angular_following_hampel_k_);
}
...
bool LineFollowController::setPlan(const nav_msgs::msg::Path& orig_global_plan)
{
  ...
  double total_required_distance =
      original_alignment_distance + original_deceleration_distance + original_acce_distance;
  ...
  else
  {
    ...
    if (current_max_speed > speed_threshold)
    {
      ...
      if (available_high_speed_distance < path_length_ * 0.3)
      {
        ...
        m_acce_distance_ = std::max(0.0, remaining_distance * 0.6);
      }
      else if (current_max_speed > speed_threshold * 1.5)
      {
        ...
        double optimized_max_speed =
            speed_threshold * 1.5 * conservative_factor + speed_threshold * 0.5 * (1.0 - conservative_factor);
...
double LineFollowController::computeLinearSpeed(double distance_to_target, double distance_to_start)
{
  ...
  if (distance_to_target < deceleration_distance_)
  {
    ...
    const double precise_stop_distance = m_work_state_ ? 0.05 : 0.08;
                                                    ...
                                                    double distance_to_start, double linear_speed)
{
  ...
  double current_cross_track_deadzone = 0.0;
  double current_yaw_deadzone = 0.0;
  double current_max_angular_vel = smooth_terrain_params_.max_angular_vel;
  ...
  {
    deadzone_factor = std::pow(deadzone_factor, 2.0);
  }
...
double LineFollowController::calculateRotationVelocity(const double& angle_diff)
{
  ...
  if (std::abs(angle_diff) < rotation_angle_threshold_)
  {
    double cosine_factor =
        rotation_angle_smooth_factor_ * (1.0 - std::cos(M_PI * std::abs(angle_diff) / rotation_angle_threshold_));
                                               ...
                                               geometry_msgs::msg::TwistStamped& cmd_vel)
{
  ...
  if (distance_to_start > m_alignment_distance_)
  {
    ...
    double current_heading_weight = adaptive_params_.current_heading_weight;
    double target_heading_weight = adaptive_params_.target_heading_weight;
  ...
  if (debug_)
  {
    ...
    switch (current_state_)
    {
      ...
      case ControlState::ALIGNING_START:
        state_name = "ALIGNING_START";
        break;
      case ControlState::FOLLOWING_PATH:
        state_name = "FOLLOWING_PATH";
        break;
      case ControlState::ALIGNING_END:
        state_name = "ALIGNING_END";
        break;
  ...
  publishControlLogData(yaw_error, cross_track_error, cmd_vel, back_follow_, in_deadzone, control_data_.deadzone_factor,
                        control_data_.d_adaptive_factor, log_max_angular_vel, log_suppression_factor,
                        log_cross_track_deadzone, log_yaw_deadzone, log_max_angular_accel,
                        control_data_.angular_acceleration, enable_imu_terrain_adaptation_);
...
void LineFollowController::initializeImuSubscription()
{
  ...
  if (!imu_node_)
  {
    ...
    imu_node_options.use_intra_process_comms(true);
...
{
  const std::chrono::microseconds loop_period(5000);
...
double LineFollowController::computeZeroCrossingRate()
{
  ...
  const double noise_threshold = 0.0017;
  ...
  double duration = std::chrono::duration_cast<std::chrono::duration<double>>(imu_history_.back().timestamp -
                                                                              imu_history_.front().timestamp)
                        .count();
...
LineFollowController::TerrainType LineFollowController::classifyTerrainWithPriority(const TerrainFeatures& features)
{
  ...
  if (debug_ && is_print)
  {
    publishTerrainLogData(features, result, 0.8);
  }
...
void LineFollowController::adaptControlParameters(TerrainType terrain_type, double confidence)
{
  ...
  terrain_data_.angle_vel_dynamic_factor = 1.0 - confidence * 0.5;
...
double LineFollowController::calculateControlStabilityScore(double current_angular_vel, double prev_angular_vel)
{
  ...
  double max_expected_change = 0.5;
...
{
  double max_acceptable_error = 0.05;
...
double LineFollowController::calculateResponseTimeScore(double yaw_error, double angular_response)
{
  ...
  double ideal_response_ratio = 2.0;
...
void LineFollowController::publishControlLogData(double yaw_error, double cross_track_error,
                                                 ...
                                                 double max_angular_vel, double suppression_factor,
                                                 double cross_track_deadzone, double yaw_deadzone,
                                                 double max_angular_accel, double current_angular_accel,
                                                 bool imu_terrain_enabled)
{
  if (!terrain_log_publisher_)
  {
    return;
  ...
  try
  {
    std::string json_message =
        createControlJsonMessage(yaw_error, cross_track_error, cmd_vel, back_follow, in_deadzone, deadzone_factor,
                                 d_adaptive_factor, max_angular_vel, suppression_factor, cross_track_deadzone,
                                 yaw_deadzone, max_angular_accel, current_angular_accel, imu_terrain_enabled);
...
std::string LineFollowController::createControlJsonMessage(double yaw_error, double cross_track_error,
                                                           ...
                                                           double d_adaptive_factor, double max_angular_vel,
                                                           double suppression_factor, double cross_track_deadzone,
                                                           double yaw_deadzone, double max_angular_accel,
