...
> 版本：2026-03-04 | 适用代码：B33+，第8节参数值基于 B39 配置
...
### 错误六：对齐阈值过严引发跟随阶段系统性偏差（B37/B38 教训）

**现象**：将 `line_cross_track_tolerance` 从 6mm 收紧至 4mm，预期对齐更精准，结果 path_01 平均偏差从 -1.27mm 骤升至 -3.88mm，`cancel>0.5` 从 18.8% 升至 48.3%，avg<3mm% 从 60.5% 跌至 50.1%。

**错误根因**：
- 对齐阈值决定机器人何时切换至跟随阶段
- 阈值收紧 → 对齐阶段花更多时间修正，但修正本身会在 **接近阈值** 时产生欠调整残差
- 最终以略微偏向一侧的姿态进入跟随 → 短路径（path_01，1.8m）全程无法消除该初始偏差
- 积分饱和 → anti_cancel 频繁触发 → ζ 骤降

**关键数据**：
```
align_tol=6mm: path_01 mean=-1.27mm, cancel>0.5=18.8%, ζ=0.601
align_tol=4mm: path_01 mean=-3.88mm, cancel>0.5=48.3%, ζ=0.565
```

**教训**：
- 对齐阈值不是"越小越好"。实测以 **6mm** 为基准；只有当样本数据显示对齐残差系统偏大时才收紧。
- 判断依据：查看跟随阶段 `ct[0]`（路径首帧横向误差），若 `|ct[0]| > 4mm` 才说明对齐不足。
- 每次修改 align_tol 后必须重点检查 path_01（最敏感，短路径几乎无自愈能力）。

### 错误七：k2_gate.ey_start 设置低于正常工作误差范围（B37 教训）

**现象**：将 `k2_gate.ey_start` 从 4.8mm 降低至 3mm，希望对大误差段更早激活 K2 衰减。结果正常跟随阶段（误差 2-4mm）大量帧触发门控，K2 被压制，ζ 骤降。

**错误根因**：
- k2_gate 的设计初衷是针对"误差已经很大（>ey_start）时 K2 项会干扰收敛"的假设
- 但正常精细跟线时横向误差本身就分布在 2-5mm 区间
- ey_start=3mm → 正常跟线时有大量帧处于"门控激活区" → K2_eff 被降至 min_scale 水平 → 系统性欠阻尼

**规则**：
```
ey_start ≥ 正常工作误差分布的 90 分位值
通常：ey_start = 1.5 × p90_目标 ≈ 4-6mm 为安全范围
绝对不应低于 3mm
```

**教训**：修改 ey_start 前，先查当前数据的 `e_y_filtered_mm` 均值和 std。若正常跟线时误差 std≈3mm，则 ey_start 至少设为 5mm，留出 1.5σ 的安全边距。

---

## 8. 快速参考：数值范围与调整步长

> **注**：当前值以 B39 配置（B36 基线恢复版）为准，历史最优为 B35（avg<3mm%=62.1%）。
...
| `q2` | 0.42 | 0.20~0.60 | ±0.04 | ↑阻尼更强但响应更慢 |
| `K1_max` | 8.0 | 5.0~12.0 | ±1.0 | ↑允许低速更激进纠偏 |
| `K1_min` | 1.5 | 1.0~3.0 | ±0.3 | ↑高速段也保留基础纠偏 |

### K2 调制参数

> **floor = anti_cancel 不变量**：`k2_min_floor_scale` 必须等于 `k2_anti_cancel.scale`，否则 floor 会覆盖 anti_cancel 机制。

| 参数 | 当前值 | 安全范围 | 步长 | 效果方向 |
|------|--------|---------|------|---------|
| `k2_gate.ey_start` | 0.0048 | **0.004~0.008** | ±0.001 | ↑扩大精细段保护区（不得低于正常工作误差分布p90） |
| `k2_gate.ey_end` | 0.0180 | 0.010~0.025 | ±0.002 | ↑扩大门控过渡区，减缓K2下降速率 |
| `k2_gate.min_scale` | 0.62 | 0.45~0.75 | ±0.05 | ↑大误差段 K2 保留更多 |
| `k2_anti_cancel.scale` | 0.45 | 0.35~0.55 | ±0.05 | ↑减弱抵消抑制效果（必须=k2_min_floor_scale） |
| `k2_anti_cancel.ey_threshold` | 0.0020 | 0.001~0.004 | ±0.0005 | ↑更大误差时才触发 |
| `k2_min_floor_scale` | 0.45 | 0.35~0.55 | ±0.05 | ↑K2 绝对下限更高（必须=anti_cancel.scale） |

### 积分参数

| 参数 | 当前值 | 安全范围 | 步长 | 效果方向 |
|------|--------|---------|------|---------|
| `Ki` | 0.42 | 0.20~0.60 | ±0.05 | ↑积分补偿更强，windup 风险更高 |
| `integral_max` | 0.015 | 0.008~0.025 | ±0.003 | ↑允许更大积分蓄积 |
...
当前配置（动态模式，q1=0.39, q2=0.42, r=1.0 → K2=1.109）：

| 速度 v | K1（理论） | K1（实际） | ζ（K2_eff=1.109） |
|--------|---------|----------|---|
| 0.25 | 2.50 | 2.50 | **0.701** ✓ |
| 0.20 | 3.12 | 3.12 | 0.703 |
| 0.15 | 4.16 | 4.16 | 0.683 |
| 0.10 | 6.25 | 6.25 | 0.703 |
| 0.07 | 8.93 | **8.0**（截幅） | 0.733 |
| 0.05 | 12.5 | **8.0**（截幅） | 0.877 |

> ζ 目标范围：**[0.65, 0.85]**。当前配置 ζ=0.70 处于理想区间。
> K2_eff 实际值受 k2_gate/anti_cancel 影响，上表为理想值（gate未激活时）。
